...
beginning = time.time()
#reset random number generator for reproducability (should make it the same as MATLAB)
np.random.seed(1337)
...
b = b+b.T#+5*np.identity(nodes) #add values to diagonal to filter for removal

b[(np.arange(0,nodes),np.arange(0,nodes))]=0 #turn diagonal values into zeros (this should be simpler to do but idk)
...
a=a.to_numpy();x=x.to_numpy();y=y.to_numpy() #I gotta stop doing it like this, but it's research code and it works
...
print('%d'%nace,'& %0.1f'%mace,'& %0.1f'% mabe,'& %0.1f'%(mabe-mace),   \
      '& %d'%nxce,'& %0.1f'%mxce,'& %0.1f'% mxbe,'& %0.1f'%(mxbe-mxce), \
      '& %d'%nyce,'& %0.1f'%myce,'& %0.1f'% mybe,'& %0.1f'%(mybe-myce), \
      '& %0.1f'% (max(mabe-mace,mxbe-mxce,mybe-myce)-min(mabe-mace,mxbe-mxce,mybe-myce)))